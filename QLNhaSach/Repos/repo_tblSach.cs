    ...
    public class repo_tblSach : Restful<tblSach>
    {
        ...
        public bool Delete(tblSach record)
        {
            try
            {
				...
				//xoá liên kết tại bảng TheLoaiSach
				command = new SqlCommand("delete TheLoaiSach where id_tblSach = @id", sqlConnection);
				...
				//xoá liên kết tại bảng ChiTietHoaDonBanSach
				command = new SqlCommand("delete ChiTietHoaDonBanSach where id_tblSach = @id", sqlConnection);
				command.Parameters.AddWithValue("@id", record.id);
